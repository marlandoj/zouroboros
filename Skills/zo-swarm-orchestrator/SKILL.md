...
2. **Retrieval**: Embeds with OpenAI `text-embedding-3-small` (1536d) and runs the same rerank + hybrid pipeline as the qdrant-rag MCP server — per-collection dense (or dense+BM25 hybrid) fan-out → Reciprocal Rank Fusion across collections → RankGPT rerank to topK. No local model / Ollama path.
3. **Context Injection**: Top 3 most relevant patterns are injected into the prompt before the task description
...
|----------|---------|-------------|
| `QDRANT_URL` | `http://127.0.0.1:6333` | Qdrant endpoint (self-hosted local) |
| `QDRANT_API_KEY` | (optional) | Qdrant API key |
| `OPENAI_API_KEY` | (required) | OpenAI key for `text-embedding-3-small` + RankGPT rerank |
| `RAG_COLLECTIONS` | (optional) | Comma-separated collection whitelist override |
