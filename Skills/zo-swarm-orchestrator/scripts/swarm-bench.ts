...
async function verifyAC(criterion: AcceptanceCriterion, workspacePath: string, output?: string): Promise<ACVerification> {
  ...
  switch (criterion.type) {
    ...
    case "test_pass": {
      const rawCmd = criterion.test_command ?? (criterion as any).command;
      if (!rawCmd) {
        return { criterion, result: "fail" as const, details: "No test_command or command specified", score: 0 };
      }
