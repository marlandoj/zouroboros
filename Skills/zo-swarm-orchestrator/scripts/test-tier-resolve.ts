...
for (const tc of cases) {
  const result = await $`bun ${SCRIPT} --json ${tc.prompt}`.text();
  let data: any;
  try {
  ...
  const tier = data.tier ?? data.complexity?.tier;
  const taskType = data.inferredTaskType ?? data.complexity?.inferredTaskType;
  const resolved = data.combo ?? data.resolvedCombo;
  const errs: string[] = [];
