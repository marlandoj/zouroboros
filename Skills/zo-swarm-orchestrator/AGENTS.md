...
> **Capability Playbook (inherit this):** every swarm task must use the four force-multipliers
> as documented in the workspace root `/home/workspace/AGENTS.md` → **"Capability Playbook"**
> section: graph-first retrieval (codebase-memory) → qdrant/RAG → **consensus-gate net-new
> logic before merge** (`--code` TS snippets, never raw diffs) → MoA panel (set
> `OPENROUTER_API_KEY`; cheap fusion = Opus ceiling at horizon 12–16). Do not re-derive these
> per task — the root file is the single source of truth.
