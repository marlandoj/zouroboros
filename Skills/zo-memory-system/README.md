...
- **Default Routing** -- Generation workloads default to OpenAI `gpt-4o-mini`; embeddings use OpenAI `text-embedding-3-small`
...
The current default routing uses OpenAI for both generation workloads and embeddings.
...
| openai:text-embedding-3-small | Vector embeddings | API | Yes (default embedding path) |
...
```bash
export ZO_EMBEDDING_MODEL="openai:text-embedding-3-small"
