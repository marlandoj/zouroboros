...
import { HierarchicalMemory, SlidingWindowMemory, MemoryItem } from "./token-optimizer";
...
import {
  ...
  type OperationalPlanGateOptions,
} from "./plan-gate-runtime";
...
interface OrchestratorConfig {
  ...
  defaultMemoryStrategy: "hierarchical" | "sliding" | "none";
  ...
  // Cascade
  cascadeMode: boolean;
  planGate?: OperationalPlanGateOptions;
}
...
function serializeCircuitBreakers(
  ...
  for (const [id, cb] of breakers) {
    const state = backpressure.get(id);
    result[id] = {
      state: cb.state,
      ...
      failureCategories: Object.fromEntries(cb.failureCategories),
      savedAt: Date.now(),
      backpressure: state ? {
  ...
  for (const [id, persisted] of Object.entries(data)) {
    ...
    breakers.set(id, {
      ...
      failureCategories: new Map(
        Object.entries(persisted.failureCategories).map(([key, value]) => [key as ErrorCategory, value]),
      ),
    });
  ...
  constructor(swarmId: string, config: OrchestratorConfig) {
    ...
    if (config.enableMemory) {
      ...
      if (config.defaultMemoryStrategy === "hierarchical") {
        ...
        this.sliding = new SlidingWindowMemory(config.crossTaskContextWindow);
      }
    }
  }
...
class SwarmOrchestrator {
  ...
  private async preflight(tasks: Task[]): Promise<{ ok: boolean; errors: string[] }> {
    ...
    const planGate = await runOperationalPlanGatePreflight(this.config.planGate);
    console.log(`  [PLAN GATE:${planGate.mode}] ${planGate.action.toUpperCase()} - ${planGate.reason}`);
    if (planGate.auditError) console.log(`  [PLAN GATE WARN] ${planGate.auditError}`);
    if (planGate.action === "hold") {
      errors.push(`Plan gate held execution: ${planGate.reason}`);
...
Consider approaching this as a "${stagnation.suggestedPersona}" would.
  ...
  private async callLocalAgent(executorId: string, bridge: string, prompt: string, timeoutSeconds: number): Promise<string> {
    ...
    const proc = spawn("bash", [bridge, prompt], { env, detached: true });
    ...
    return new Promise((resolve, reject) => {
      ...
      timeoutId = setTimeout(() => {
        ...
        forceKillId = setTimeout(() => terminateProcessGroup("SIGKILL"), 2000);
  ...
  private createEpisode(successful: number, failed: number, durationMs: number): void {
    try {
      ...
      const outcome = failed === 0 ? "success" : "failure";
...
async function main(): Promise<void> {
  ...
  if (args.length < 1) {
    ...
    console.log("  --plan-gate-mode M   disabled|advisory|shadow|enforce");
    console.log("  --plan-gate-plan P   Structured plan artifact path");
    console.log("  --plan-gate-receipt P Approval receipt path");
    console.log("  --plan-gate-ledger P Audit ledger path");
    process.exit(1);
  }
  ...
  const planGate: OperationalPlanGateOptions = {};
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    switch (a) {
      ...
      case "--4-signal":
        ...
        break;
      case "--plan-gate-mode":
        if (i + 1 < args.length) planGate.mode = args[++i];
        break;
      case "--plan-gate-plan":
        if (i + 1 < args.length) planGate.planPath = args[++i];
        break;
      case "--plan-gate-receipt":
        if (i + 1 < args.length) planGate.receiptPath = args[++i];
        break;
      case "--plan-gate-ledger":
        if (i + 1 < args.length) planGate.ledgerPath = args[++i];
        break;
    }
  }

  config.planGate = planGate;
