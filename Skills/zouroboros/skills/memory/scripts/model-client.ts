...
async function openaiGenerate(opts: GenerateOptions): Promise<GenerateResult> {
  ...
  // USD per 1M tokens. Unversioned historical telemetry used an incorrect
  // $0.07/$0.28 per 1K formula and is normalized by the dashboard reader.
...
async function openaiCompatibleGenerate(
  ...
  maxTokens: number,
  signal?: AbortSignal
): Promise<{ content: string; latency_ms: number; cost_usd: number }> {
  const t0 = Date.now();
  ...
  const cost = (data.usage?.prompt_tokens ?? 0) * 0.0 + (data.usage?.completion_tokens ?? 0) * 0.0;
  return {
...
async function openrouterGenerate(opts: GenerateOptions): Promise<GenerateResult> {
  ...
  // Default 30s hard timeout for OpenRouter calls; extraction workload was hitting 84s outliers
  ...
  return { content: result.content, latency_ms: result.latency_ms, cost_usd: result.cost_usd, provider: "openrouter", model };
}
