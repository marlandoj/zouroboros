...
// Always skip vector embeddings — use FTS-only to avoid OpenAI quota on exhausted keys
process.env["ZO_MODEL_EMBEDDING"] = "disabled";
...
async function hybridSearch(
  query: string,
  options: { persona?: string; limit?: number; useHyde?: boolean; useGraph?: boolean; skipVector?: boolean; expandSessions?: boolean } = {}
): Promise<Array<{ entry: MemoryEntry; score: number; sources: string[]; expandedFrom?: string }>> {
  const db = await initDb();
  const { persona, limit = 10, useHyde = true, useGraph = true, skipVector = false, expandSessions = false } = options;
