...
function createTestDb(): Database {
  ...
  for (const entity of ["project.ffb", "system.memory", "config.routing", "persona.hermes", "tool.qdrant"]) {
    db.prepare(
...

{
  ...
  assert(
    result?.corrected_value.includes("[[system.memory]]") && result?.corrected_value.includes("[[tool.qdrant]]"),
...
{
  const result = autoCorrectWikilinks("The project.ffb system uses tool.qdrant", db, "project.ffb");
  ...
  assert(
    result?.corrected_value.includes("[[tool.qdrant]]"),
