 ...
 *   2. MAP the human annotations in reconciled-holdout.json into a PROPOSED
 ...
 *      calibration seed AND, SEPARATELY, over the held-out set (anti-Goodhart —
 *      the held-out is reported but is never used to tune the proposal).
 *
...
if (import.meta.main) {
  ...
  const holdoutDoc = fs.existsSync(HOLDOUT)
    ? (JSON.parse(fs.readFileSync(HOLDOUT, "utf8")) as { cases?: HoldoutCase[] })
    : { cases: [] };
  const stats = summarizeAnnotations(holdoutDoc.cases || []);
  ...
  console.log(
    `✓ rubric snapshot : ${vNPath}\n` +
      ...
      `  held-out: ${stats.total} cases — overEscalateClean=${stats.overEscalateClean}, ` +
