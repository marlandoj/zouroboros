...
function classify(raw: RawModel): ClassifiedModel {
  ...
  const family = hfId.split("/")[0] || "unknown";
  ...
  return { ...raw, family, tier, promptCost, completionCost };
}
...
export function buildChain(primary: ClassifiedModel, all: ClassifiedModel[]): string[] {
  ...
  candidates.sort((a, b) => {
    const aDiff = a.family !== primary.family ? 1 : 0;
    const bDiff = b.family !== primary.family ? 1 : 0;
 ...
 * caches (synthetic, openrouter, opencode). The consensus gate uses this to
...
export function lookupPricing(modelId: string): { promptCost: number; completionCost: number } | null {
  const stripped = modelId.replace(/^(hf:|oc:|xai:)/, "");
  const caches = [
