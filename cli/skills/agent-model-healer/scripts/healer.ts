...
const WORKSPACE = process.env.ZOUROBOROS_WORKSPACE || process.env.ZO_WORKSPACE || process.cwd();
// Skill-bundle asset — resolve relative to this script so it works wherever the skill is
// installed. Prefer the operator's real fallback-chain.json; a fresh install only ships the
// .example.json, so fall back to that. ZOUROBOROS_HEALER_CONFIG overrides both.
const CONFIG_REAL = join(import.meta.dir, "../assets/fallback-chain.json");
const CONFIG_EXAMPLE = join(import.meta.dir, "../assets/fallback-chain.example.json");
const CONFIG_PATH =
  process.env.ZOUROBOROS_HEALER_CONFIG ||
  (existsSync(CONFIG_REAL) ? CONFIG_REAL : CONFIG_EXAMPLE);
const STATE_PATH = join(WORKSPACE, ".zouroboros/healer-state.json");
...
const OPEN_WEIGHT_LABEL_HINTS = ["kimi", "moonshot", " k2", "k2.", "qwen", "deepseek", "llama", "minimax"];
...
async function cmdProbe() {
  if (!zoAvailable()) return bareBoxNotice("probe");
...
async function cmdAuto() {
  if (!zoAvailable()) return bareBoxNotice("auto");
  ...
  // Title match is the portable guard; HEALER_AGENT_ID env pins an exact
  // agent id when the runtime propagates env (operator-specific, optional).
  const HEALER_AGENT_ID = process.env.HEALER_AGENT_ID || "";
...
async function cmdRun(agentsJson?: string) {
  if (!zoAvailable()) return bareBoxNotice("run");
