...
- **Transport Abstraction**: ACP, bridge, and local transports
...
| Steward (Memory Manager) | Knowledge (memory graph) | Daily |
...
- **Memory Capture & Briefing** — Conversation artifacts → `conversation-capture` → provider-routed extraction/embeddings → SQLite facts + graph → memory gate → session briefing / Memory Manager synthesis
...
## Consensus Gate Failover

The Consensus Gate validates procedure evolutions across three frontier models in parallel. Each model runs on **synthetic.new** (primary) and fails over to the **same model on OpenRouter** when synthetic returns an empty or errored verdict, so vendor-side outages never poison consensus. The `xai:` (grok) rung routes to x.ai directly and does not fail over.

<p align="center">
  <img src="../../assets/consensus-gate-failover.png" alt="Consensus Gate — synthetic.new primary with OpenRouter same-model failover" width="100%" />
</p>
