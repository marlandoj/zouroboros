...
async function main() {
  ...
  if (!stage || stage === '2') {
    ...
    for (const criterion of result.criteria) {
      ...
      const rt = criterion.reflectionTokens;
      const tokens = [
      ...
      console.log(`${status} [conf:${(criterion.confidence * 100).toFixed(0)}%] ${criterion.name}`);
      console.log(`     tokens: ${tokens}  evidence: ${criterion.evidence || 'No evidence found'}`);
    }

    console.log(`\nScores:`);
    console.log(`  AC Compliance:   ${(result.acCompliance * 100).toFixed(0)}%`);
    console.log(`  Token Agreement: ${(result.tokenAgreement * 100).toFixed(0)}%`);
