...
async function proposeChange(
  ...
  executor: string,
  actionId: string
): Promise<{ hypothesis: string; costUSD: number | null }> {
...
Do NOT modify any read-only files. Do NOT install new packages.`;
  ...
  // ZOU-889 — point the bridge at a per-action result file so we can read its
  // measured usage cost. actionId is sanitized for filesystem safety; the file
  // lives beside the cost ledger (workDir/.autoloop). Any stale file from a
  // prior run is removed first, so a bridge that fails to write => absent file
  // => unknown cost => fail closed (never a stale read).
  const resultPath = join(workDir, ".autoloop", `result-${actionId.replace(/[^a-zA-Z0-9_-]/g, "_")}.json`);
  mkdirSync(dirname(resultPath), { recursive: true });
  try { if (existsSync(resultPath)) unlinkSync(resultPath); } catch {}
  ...
  // ZOU-889 — capture the measured usage cost the bridge wrote. Only the
  // claude-code bridge emits metrics.totalCostUsd; codex/gemini omit it =>
  // parseBridgeCost => null (unknown, honest). A missing/unreadable file =>
  // null => fail closed under enforce, advisory under the default-off flag.
  let costUSD: number | null = null;
  try {
    if (existsSync(resultPath)) {
      costUSD = parseBridgeCost(readFileSync(resultPath, "utf-8"));
    }
  } catch {}

  return { hypothesis, costUSD };
}
...
async function main() {
  ...
  // ZOU-881 — measured cost circuit breaker. Advisory by default; set
  // AUTOLOOP_COST_ENFORCE=1 to fail closed. Ticket Protected Behavior forbids
  // enabling production behavior here, so enforcement stays off by default.
  const costEnforce = process.env.AUTOLOOP_COST_ENFORCE === "1";
  const costLedgerPath = join(workDir, ".autoloop", "cost-ledger.jsonl");
  const costAcc: CostAccumulator = values.resume
    ? replayCostLedger(costLedgerPath)
    : newAccumulator();
  state.totalCostUSD = costAcc.cumulativeUSD;
  // Usage seam: a future executor-usage source (broad ZOU-881 follow-up) sets
  // AUTOLOOP_ACTION_COST_USD per action. Absent today => unknown => fail closed.
  const readActionCostUSD = (): unknown => {
    const raw = process.env.AUTOLOOP_ACTION_COST_USD;
  ...
  while (true) {
    ...
    // Cost ceiling — stop before authorizing the next metered action (ZOU-881).
    {
      const ceiling = checkCostCeiling(costAcc.cumulativeUSD, config.constraints.maxCostUSD);
      if (ceiling.stop) {
        appendCostLedger(costLedgerPath, {
          ts: new Date().toISOString(),
          type: "stop",
          cumulativeUSD: costAcc.cumulativeUSD,
          reason: ceiling.reason,
        });
        if (costEnforce) {
          writeSummary(config, state, workDir, ceiling.reason || "Cost ceiling reached");
          break;
        }
        console.log(`[cost] ADVISORY — ${ceiling.reason} (set AUTOLOOP_COST_ENFORCE=1 to stop)`);
      }
    }
    ...
    try {
      const proposal = await proposeChange(config, state, workDir, executor, `exp-${expNum}:attempt`);
      hypothesis = proposal.hypothesis;
      attemptCostUSD = proposal.costUSD;
    } catch (err: any) {
    ...
    // Account for the proposal's metered cost (ZOU-881). actionId is stable
    // across restart (same experiment => same id), so replay never double-counts.
    {
      const actionId = `exp-${expNum}:attempt`;
      // ZOU-889 — feed the measured bridge cost; fall back to the env seam only
      // when capture yielded nothing (absent file / no cost => null).
      const decision = recordSpend(costAcc, {
        actionId,
        kind: "attempt",
        costUSD: attemptCostUSD ?? readActionCostUSD(),
        experiment: expNum,
      });
      state.totalCostUSD = decision.cumulativeUSD;
      appendCostLedger(costLedgerPath, {
        ts: new Date().toISOString(),
        type: "spend",
        actionId,
        ...
        cumulativeUSD: decision.cumulativeUSD,
        reason: decision.reason,
      });
      if (!decision.authorized && costEnforce) {
        writeSummary(config, state, workDir, decision.reason || "Unknown metered cost — failing closed");
        break;
      }
      if (!decision.authorized) {
        console.log(`[cost] ADVISORY — ${decision.reason} (set AUTOLOOP_COST_ENFORCE=1 to fail closed)`);
      }
    }
    ...
    if (result.crashed) {
      ...
      while (crashFixAttempts < maxCrashFixes) {
        ...
        try {
          ...
          const retryActionId = `exp-${expNum}:retry-${crashFixAttempts}`;
          ...
          }, workDir, executor, retryActionId);
          hypothesis = proposal.hypothesis;

          // ZOU-889 — account for the retry proposal's metered cost. A distinct
          // actionId per crash-fix attempt means replay never double-counts, and
          // an unknown cost fails closed under enforce exactly like the attempt.
          {
            const decision = recordSpend(costAcc, {
              actionId: retryActionId,
              kind: "retry",
              costUSD: proposal.costUSD ?? readActionCostUSD(),
              experiment: expNum,
            });
            state.totalCostUSD = decision.cumulativeUSD;
            appendCostLedger(costLedgerPath, {
            ...
            if (!decision.authorized && costEnforce) {
              costStop = decision.reason || "Unknown retry cost — failing closed";
              break;
            }
            if (!decision.authorized) {
              console.log(`[cost] ADVISORY — ${decision.reason} (set AUTOLOOP_COST_ENFORCE=1 to fail closed)`);
      ...
      // ZOU-889 — an unknown retry cost under enforce stops the run fail-closed.
      if (costStop) {
        writeSummary(config, state, workDir, costStop);
        break;
      }
