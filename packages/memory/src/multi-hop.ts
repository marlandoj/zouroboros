...
const HYDE_MODEL = process.env.ZO_HYDE_MODEL || "gpt-4o-mini";
...
async function modelGenerate(prompt: string): Promise<string> {
  try {
...
What additional information would help answer the original question? Reply with 1-2 specific search terms.`;
  const refined = await modelGenerate(prompt);
...
Provide a 2-sentence answer to the question.`;
  const reasoning = await modelGenerate(reasoningPrompt);
