#!/usr/bin/env bun
/**
 * memory-gate.ts — LLM-powered memory relevance filter
 *
 * Takes a user message, uses OpenAI (gpt-4o-mini) via model-client to:
...
export async function injectSessionBriefing(personaSlug: string): Promise<string | null> {
  ...
  try {
    const domain = await resolvePersonaDomain(personaSlug);
...
export async function shouldInjectMemory(taskText: string): Promise<GateDecision> {
  ...
  // Tier 3: LLM classifier (200ms timeout)
  try {
    const gate = await Promise.race([
      callLlmClassifier(taskText),
...
async function main() {
  ...
  try {
    ...
    // Wikilink fast-path: if message contains [[entity]], search directly without LLM
    ...
    // Keyword heuristic: deterministic pre-check before LLM
