 ...
 * - Vector embeddings (text-embedding-3-small via OpenAI)
...
import { generate as modelGenerate, embeddings as mcEmbeddings } from "./model-client";
...
async function initDb(): Promise<Database> {
  ...
  // M9 STaR: rationale + outcome columns on procedure_episodes
  // Idempotent — older DBs predate the inline schema above.
...
async function evolveProcedure(procedureName: string): Promise<Procedure | null> {
  ...
  const generalFailures = stepExecutors.length > 0
    ? db.prepare(`
  ...
  // STaR (Zelikman 2022): generate a reasoning chain ("rationale") that explains
  // WHY the proposed steps address the failures, then the steps themselves.
  // Persisted to procedure_episodes so future evolutions can learn from prior reasoning.
  ...
  const prompt = `You are optimizing a multi-step workflow procedure. First reason step-by-step about WHY recent failures occurred, then propose improved steps.
...
Keep the same general structure but adjust executors, timeouts, or add fallbacks based on the failure pattern you identified.`;
  ...
  try {
    ...
    const constitutionGatePath = join(getWorkspaceRoot(), "Skills/zouroboros-governance/scripts/constitution-gate.ts");
    execFileSync("bun", [
    ...
    // Consensus gate validation (Phase 1: multi-vendor code review)
    console.log(`🔄 Validating evolved procedure steps through consensus gate...`);
    const stepCode = JSON.stringify(validSteps, null, 2);
    const consensusGatePath = join(getWorkspaceRoot(), "Skills/consensus-gate/scripts/consensus-gate.ts");
    try {
      ...
      const validationResult = JSON.parse(validationOutput) as any;
      ...
      if (validationResult.consensus.pass === null) {
        throw new Error(
          `Consensus gate returned a split verdict for procedure evolution (${validationResult.id || "unknown id"})`
        );
      } else {
        // Unanimous pass
        console.log(`✅ Consensus gate passed (all vendors agree)`);
      }
    } catch (gateErr: any) {
    ...
    // Persist rationale on every failure episode used as input — STaR memory.
    // Outcome 'pending' marks it as an evolution-induced edit; downstream success/failure
    // tracking will overwrite once the new procedure version executes.
    if (rationale) {
      ...
      for (const ep of allFailures) {
        ...
        // Also link generalFailures (entity-overlap matches not yet in procedure_episodes)
        db.prepare(
          `INSERT OR IGNORE INTO procedure_episodes (procedure_id, episode_id, rationale, outcome)
           VALUES (?, ?, ?, 'pending')`
        ).run(current.id, epId, rationale);
      }
    }
    ...
    console.log(`  Provider: ${result.provider}/${result.model} (cost $${result.cost_usd.toFixed(5)})`);
    return evolved;
  } catch (err) {
...
async function stats(): Promise<void> {
  ...
  console.log(`  LLM provider: OpenAI (gpt-4o-mini via model-client)`);
...
Procedures options:
  ...
  --evolve <name>      Evolve a procedure via STaR rationale + LLM step proposal
