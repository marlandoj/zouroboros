 ...
 * process alive. Exposes HTTP endpoints for:
 *   POST /gate     — classify message + return memory context
 *   POST /briefing — generate session briefing
 *   GET  /health   — uptime + provider status + backend statuses
 *
 * Generation uses OpenAI (gpt-4o-mini) via model-client. No Ollama.
 * Multi-backend support: personas route to separate DB files via backends.json.
 *
 * Register as a user service:
 *   entrypoint: bun <workspace-root>/Skills/zo-memory-system/scripts/memory-gate-server.ts
...
// `domain-map.ts` is an OPTIONAL dependency: it maps named personas to business
// domains and is intentionally absent from the public source tree. Load it lazily and
// degrade to no domain scoping when it is missing. The catch is narrowed to
// module-not-found so a genuine error in the in-house domain-map still surfaces.
let _domainMapPromise: Promise<{ getPersonaDomain(persona: string): string } | null> | undefined;
function loadDomainMap() {
  if (_domainMapPromise === undefined) {
    _domainMapPromise = import("./domain-map.ts").catch((e: any) => {
      const code = e?.code ?? "";
      const msg = e?.message ?? "";
      if (code === "ERR_MODULE_NOT_FOUND" || /Cannot find (module|package)/i.test(msg)) return null;
      throw e;
    });
  }
  return _domainMapPromise;
}
async function resolvePersonaDomain(persona: string): Promise<string | null> {
  const mod = await loadDomainMap();
  return mod ? mod.getPersonaDomain(persona) : null;
}
...
const MEMORY_SCRIPT = join(getWorkspaceRoot(), "Skills/zo-memory-system/scripts/memory.ts");
const MAX_RESULTS = 5;
...
const BACKENDS_CONFIG_PATH = join(getWorkspaceRoot(), ".zo/memory/backends.json");
const DEFAULT_DB_PATH = process.env.ZO_MEMORY_DB || join(getWorkspaceRoot(), ".zo/memory/shared-facts.db");
...
const DEFAULT_DB_FOR_SEARCH = join(getWorkspaceRoot(), ".zo/memory/shared-facts.db");
...
async function callLlmClassifier(message: string): Promise<GateResponse> {
...
async function handleGate(req: GateRequest): Promise<GateResult> {
  ...
  try {
    ...
    if (persona && !isBriefingFresh(persona)) {
      try {
        const domain = await resolvePersonaDomain(persona);
        const effectiveDomain = !domain || domain === "shared" || domain === "personal" ? undefined : domain;
    ...
    // LLM classifier (OpenAI gpt-4o-mini via model-client)
    const gate = await callLlmClassifier(message);
    ...
    const INLINE_CAPTURE_SCRIPT = join(getWorkspaceRoot(), "Skills/zo-memory-system/scripts/inline-capture.ts");
  ...
  async fetch(req) {
    ...
    if (url.pathname === "/briefing" && req.method === "POST") {
      try {
        ...
        const domain = await resolvePersonaDomain(body.persona);
        const effectiveDomain = !domain || domain === "shared" || domain === "personal" ? undefined : domain;
...
console.log(`[memory-gate-server] Provider: OpenAI (gpt-4o-mini via model-client)`);
