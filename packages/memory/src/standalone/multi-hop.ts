...
async function llmGenerate(prompt: string): Promise<string> {
  try {
    const result = await generate({ prompt, workload: "hyde", temperature: 0.1, maxTokens: 300 });
...
What additional information would help answer the original question? Reply with 1-2 specific search terms.`;
  const refined = await llmGenerate(prompt);
...
Provide a 2-sentence answer to the question.`;
  const reasoning = await llmGenerate(reasoningPrompt);
