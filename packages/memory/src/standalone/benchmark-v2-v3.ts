#!/usr/bin/env bun
/**
 * benchmark-v2-v3.ts — Head-to-head benchmark: v2.0 vs v3.0 (HISTORICAL)
 *
 * DEPRECATED: This benchmark was written against Ollama-local models
 * (nomic-embed-text, qwen2.5:1.5b, qwen2.5:7b). Ollama was removed in favor of
 * OpenAI (gpt-4o-mini + text-embedding-3-small) in April 2026 (PR #87).
 * This file is kept for historical reference only. Running it without
 * --skip-ollama will fail because Ollama is no longer available.
 *
...
function seedDatabase(db: Database): SeedFact[] {
  const facts: SeedFact[] = [
    // Demo shop cluster (fictional botanicals e-commerce — should be linked)
    { id: "", entity: "project.demo-shop", key: "name", value: "Acme Botanicals (fictional brand) e-commerce website built on Zo hosting", category: "project", decay: "permanent" },
    { id: "", entity: "project.demo-shop", key: "stack", value: "React frontend with Hono API routes on zo.space, Stripe for payments", category: "fact", decay: "stable" },
    { id: "", entity: "project.demo-shop", key: "status", value: "Sprint N remediation in progress, A of B tasks passing", category: "fact", decay: "active" },
    { id: "", entity: "decision.demo-shop-hosting", key: "choice", value: "Selected Zo hosting over a managed storefront for full control and lower costs", category: "decision", decay: "permanent" },
    { id: "", entity: "decision.demo-shop-payments", key: "choice", value: "Using Stripe Connect for payment processing with webhook integration", category: "decision", decay: "permanent" },
    { id: "", entity: "project.demo-shop", key: "seo-audit", value: "Completed SEO audit showing missing meta descriptions on N product pages", category: "fact", decay: "active" },
    // Memory system cluster (should be linked)
    { id: "", entity: "system.memory", key: "version", value: "Hybrid SQLite plus vector search with text-embedding-3-small embeddings via OpenAI", category: "fact", decay: "stable" },
    ...
    { id: "", entity: "system.memory", key: "gate", value: "LLM-powered memory gate (OpenAI gpt-4o-mini) filters 40-60% of messages saving tokens", category: "fact", decay: "stable" },
    // User preferences (isolated/orphan)
    { id: "", entity: "user", key: "name", value: "Sample User, based in a US metro area", category: "preference", decay: "permanent" },
    { id: "", entity: "user", key: "timezone", value: "America/Los_Angeles, observes daylight saving time", category: "preference", decay: "permanent" },
    ...
    { id: "", entity: "system.zo", key: "memory-provider", value: "Memory system uses OpenAI gpt-4o-mini for extraction/gate and text-embedding-3-small for embeddings", category: "fact", decay: "stable" },
    // Financial
    ...
    { id: "", entity: "project.demo-finance", key: "name", value: "Demo Finance (fictional brand) website review and performance audit", category: "project", decay: "active" },
    { id: "", entity: "project.demo-finance", key: "status", value: "Comprehensive executive report delivered Q1 of test year", category: "fact", decay: "active" },
...
function seedGraphLinks(db: Database, facts: SeedFact[]): number {
  ...
  // Demo-shop cluster: link project facts to decisions
  const shopFacts = facts.filter(f => f.entity.startsWith("project.demo-shop") || f.entity.startsWith("decision.demo-shop"));
  for (let i = 0; i < shopFacts.length; i++) {
    for (let j = i + 1; j < shopFacts.length; j++) {
      const relation = shopFacts[j].entity.startsWith("decision") ? "informed_by" : "related";
      db.prepare("INSERT OR IGNORE INTO fact_links (source_id, target_id, relation, weight) VALUES (?, ?, ?, ?)")
        .run(shopFacts[i].id, shopFacts[j].id, relation, 0.8);
      linkCount++;
    }
  }
  ...
  // Cross-cluster: demo-shop → swarm (swarm reviews the shop)
  const shopStatus = facts.find(f => f.entity === "project.demo-shop" && f.key === "status");
  const swarmExec = facts.find(f => f.entity === "system.swarm" && f.key === "executors");
  if (shopStatus && swarmExec) {
    db.prepare("INSERT OR IGNORE INTO fact_links (source_id, target_id, relation, weight) VALUES (?, ?, ?, ?)")
      .run(swarmExec.id, shopStatus.id, "reviews", 0.5);
    linkCount++;
  }
...
async function benchmarkGraphSearch(db: Database, facts: SeedFact[]) {
  ...
  const queries = [
    { q: "demo shop site hosting decision", expectedCluster: "demo-shop", description: "Cluster query (demo shop)" },
...
async function benchmarkMemoryGate(db: Database) {
  ...
  const messages = [
    ...
    { msg: "what did we decide about demo shop hosting?", expect: "memory", label: "Project decision" },
...
async function benchmarkAutoCapture(db: Database) {
  ...
  const transcript = `User: Let's discuss the demo shop site migration.
Assistant: Sure, I'll review the current state. The demo shop is currently hosted on Zo with React and Hono. We migrated from the old static site in January.

User: Right. We decided to use Stripe Connect for payments instead of the managed-storefront integration. Can you remind me why?
Assistant: The decision was made because Stripe Connect gives us full control over the checkout flow, lower transaction fees at 2.9% plus 30 cents, and direct webhook integration with our Zo API routes. The managed storefront would have required their payment gateway and limited our customization.
...
- entity format: "category.subject" (e.g., "project.demo-shop", "user", "decision.hosting")
...
function generateReport() {
  ...
  const reportPath = join(getWorkspaceRoot(), "Skills/zo-memory-system/BENCHMARK_REPORT.md");
