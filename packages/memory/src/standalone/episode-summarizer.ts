...
// ─── Summarization via model-client ──────────────────────────────────────────
...
Respond ONLY with valid JSON (no markdown, no explanation):
{
  ...
  const result = await generate({ prompt, workload: "summarization", json: true, temperature: 0.3, maxTokens: 400 });
...
export async function compressEpisodes(
  ...
  // Generate compressed summary via OpenAI (gpt-4o-mini)
