...
import { generate, embeddings as mcEmbeddings } from "./model-client";
import {
  createEpisodeRecord,
...
const EMBEDDING_MODEL = "text-embedding-3-small"; // OpenAI — must match reembed-openai.ts

const MIN_CONFIDENCE = 0.6;
const MIN_VALUE_LENGTH = 10;
const MAX_FACTS_PER_CAPTURE = 10;
const MAX_TRANSCRIPT_TOKENS = 3000;
...
export async function extractFactsFromText(text: string, _model?: string): Promise<CapturedFact[]> {
  const prompt = `${EXTRACTION_PROMPT}\n\nTranscript:\n---\n${text.slice(0, MAX_TRANSCRIPT_TOKENS)}\n---`;

  try {
    const result = await generate({ prompt, workload: "extraction", json: true });
...
export async function extractAndStoreFacts(
  ...
  // Extract facts via OpenAI (gpt-4o-mini) through model-client
  ...
  if (!options.dryRun) {
    ...
    `).run(
      ...
      contradictions, "openai:gpt-4o-mini", duration_ms, options.captureMode
    );
  }
