#!/usr/bin/env bun
/**
 * embedding-benchmark.ts — Embedding Model Benchmarking (HISTORICAL)
 * MEM-202: Embedding Model Selection
 *
 * DEPRECATED: This file benchmarked Ollama-local embedding models
 * (nomic-embed-text, mxbai-embed-large, all-MiniLM-L6-v2).
 * Ollama was removed in favor of OpenAI (text-embedding-3-small) in April 2026 (PR #87).
 * This file is kept for historical reference only and will not produce useful results
 * without an Ollama instance running locally.
 *
 * Usage:
...
`);
  ...
  if (command === "compare" || command === "benchmark") {
    ...
    const configPath = join(getWorkspaceRoot(), ".zo/memory/.env");
