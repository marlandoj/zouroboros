...
const OPENAI_CONFIG: MemoryConfig = { ...BASE_CONFIG };

let fetchCallCount = 0;
const originalFetch = globalThis.fetch;

function mockOpenAIFetch(): void {
  globalThis.fetch = async (_url: string | URL | Request, _opts?: RequestInit) => {
    fetchCallCount++;
    return new Response(JSON.stringify({ data: [{ embedding: MOCK_EMBEDDING }] }), {
...
describe('ECC-010: Memory Explosion Throttling', () => {
  ...
  test('normal flow — first call hits provider', async () => {
  ...
  test('dedup hit — same content within cooldown skips provider', async () => {
