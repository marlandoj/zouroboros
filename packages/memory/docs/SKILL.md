...
description: Hybrid SQLite + Vector persona memory system for Zo Computer. Episodic memory with temporal queries, graph-boosted search, BFS path finding, knowledge gap analysis, auto-capture pipeline. Gives personas persistent memory with semantic search (text-embedding-3-small), HyDE query expansion (gpt-4o-mini), LLM-powered memory gate, 5-tier decay, swarm integration, context budget tracking, cross-persona sharing, conflict resolution, and multi-hop retrieval. Requires OpenAI API key.
compatibility: Created for Zo Computer. Requires Bun and OPENAI_API_KEY.
metadata:
  author: zouroboros-contributors
  updated: 2026-03-29
  version: 4.0.0
---
...
- **Procedural memory** — Versioned workflow patterns with step sequences, success/failure tracking, and LLM-powered evolution
...
- **HyDE expansion** — gpt-4o-mini query rewriting for vague searches (parallelized)
...
- **Vector embeddings** — text-embedding-3-small (1536d) via OpenAI
...
- **Memory gate** — LLM-powered relevance filter (memory-gate.ts) that classifies messages before searching
...
- **Health checks** — OpenAI API key validation and model-client connectivity at startup
- **Fetch timeouts** — Timeouts on all model-client calls (prevents indefinite hangs)
- **Scheduled maintenance** — Hourly prune/decay automation
- **Checkpoint system** — Save/restore task state
- **Graceful fallback** — Works without OPENAI_API_KEY (FTS5 only, no LLM workloads)
...
# Or add to /root/.zo_secrets for persistence
```

### Model Selection

All LLM workloads use OpenAI via `model-client.ts`. No local model installation required.

| Model | Purpose | Notes |
|-------|---------|-------|
| `gpt-4o-mini` | Gate, extraction, summarization, HyDE, briefing | Default for all LLM workloads |
| `text-embedding-3-small` | Vector embeddings (1536d) | Default embedding model |

Override any workload model via env vars (see DEPLOYMENT.md).
...
| **With HyDE** | ~1-2s | Vague/conceptual queries (e.g., "that thing about data safety") |
...
cd <workspace-root>/Skills/zo-memory-system
...
# Check memory system health (OpenAI API key, DB connectivity)
...
1. Transcript is sent to gpt-4o-mini for structured extraction
...
Procedural memory captures reusable workflow patterns as versioned step sequences. Procedures track success/failure rates and can self-evolve via LLM (gpt-4o-mini) when failures accumulate.
...
# Force LLM-powered evolution based on failure episodes
bun scripts/memory.ts procedures --evolve "site-review"
```

### Procedure Evolution

When a procedure accumulates failures, `--evolve` uses OpenAI (gpt-4o-mini) to analyze linked failure episodes and suggest improved steps. The evolved procedure:
...
**Hosted service:** `https://zo-memory-mcp-<your-handle>.zocomputer.io`
- Service ID: see your Zo hosting panel
- Local port: 48400
- Health: `/health`
...
```json
{
  "mcpServers": {
    "zo-memory": {
      "command": "bun",
      "args": ["<workspace-root>/Skills/zo-memory-system/scripts/mcp-server.ts"]
    }
  }
}
```
...
The memory gate (`scripts/memory-gate.ts`) uses OpenAI (gpt-4o-mini) to classify incoming messages and decide whether memory context should be injected. It now prioritizes likely continuation-style turns first, using blended continuation recall before falling back to the standard memory search path.
...
4. Otherwise the LLM classifier (gpt-4o-mini) classifies whether stored memory is needed
...
```
Run: bun <workspace-root>/Skills/zo-memory-system/scripts/memory-gate.ts "<message>"
...
```bash
export ZO_MODEL_GATE="openai:gpt-4o-mini"  # Default gate model
export OPENAI_API_KEY=sk-...
```

No warm-up or keep-alive is needed — OpenAI API calls have no cold-start penalty.
...
│   ├── fact_embeddings      # Vector embeddings (1536d, text-embedding-3-small)
...
└── scripts/
    ...
    ├── memory-gate.ts       # LLM-powered relevance gate (v2.3)
...
export ZO_MODEL_GATE="openai:gpt-4o-mini"       # Default gate model
export ZO_MODEL_EXTRACTION="openai:gpt-4o-mini" # Default extraction model
export ZO_MODEL_HYDE="openai:gpt-4o-mini"       # Default HyDE model
export ZO_MODEL_EMBEDDING="openai:text-embedding-3-small"  # Default embedding
...
| 3.3.1 | 2026-03-07 | HTTP MCP server (`mcp-server-http.ts`) — Streamable HTTP transport via Bun.serve(), registered as Zo hosted service, HTTPS at `zo-memory-mcp-<your-handle>.zocomputer.io`, configured in Claude Code / Gemini / Codex / workspace .mcp.json, bridge script updated with zo-memory tool permissions |
...
| 3.2.0 | 2026-03-07 | Procedural memory (versioned workflow storage, CRUD, feedback, LLM evolution), cognitive profiles (episode IDs, failure patterns, entity affinities in executor-history.json), orchestrator v4.5 integration (auto-episode on swarm completion, 6-signal composite routing with procedure + temporal scores), MCP server (5 tools: search, store, episodes, procedures, cognitive_profile), import pipeline (ChatGPT, Obsidian, markdown), enhanced CLI (profile, import, mcp commands) |
...
| 2.3.0 | 2026-03-03 | Memory gate (memory-gate.ts), always-on context injection via Zo rules, gate-filtered token savings for swarm workflows |
| 2.2.0 | 2026-02-27 | Health checks, fetch timeouts, prune/decay/consolidate/link/graph commands, vector pre-filtering, adaptive decay, associative routing, PRAGMA busy_timeout |
| 2.1.0 | 2026-02-22 | Parallelized HyDE/FTS/embedding execution, performance docs |
| 2.0.0 | 2026-02-19 | Hybrid SQLite + Vector search, HyDE query expansion, semantic retrieval, nomic-embed-text via Ollama (pre-migration baseline) |
