...
const COST_RANKING = ['hermes', 'opencode', 'gemini', 'codex', 'claude-code'];
...
const COMPLEXITY_AFFINITY: Record<string, Record<string, number>> = {
  ...
  opencode: { trivial: 0.85, simple: 0.90, moderate: 0.90, complex: 0.85 },
...
export class RoutingEngine {
  ...
  route(task: Task, complexityTier: ComplexityTier, options?: RouteOptions): RouteDecision {
    ...
    if (options?.roleExecutorId) {
      const roleExecutor = candidates.find(e => e.id === options.roleExecutorId);
      if (roleExecutor) {
        const cb = this.context.circuitBreakers.get(roleExecutor.id);
        if (cb.getState().state !== 'OPEN') {
          return {
            executorId: roleExecutor.id,
            executorName: roleExecutor.name,
            compositeScore: 1.0,
            breakdown: {
              capability: 1.0,
              health: 1.0,
    ...
    const use8Signal = !this.context.useSixSignal && (options?.budget || options?.roleExecutorId);
  ...
  private compute8SignalScore(
    ...
    const temporalScore = cbState.lastSuccess > 0
      ? Math.max(0, 1 - (Date.now() - cbState.lastSuccess) / (1000 * 60 * 60))
      : 0.5;
  ...
  private computeBudgetScore(executorId: string, budget?: BudgetSnapshot | null): number {
    ...
    // Prefer cheaper executors (lower rank index) when budget is tight
    const costFactor = (COST_RANKING.length - costRank) / COST_RANKING.length;
  ...
  private computeCapabilityScore(executor: ExecutorCapability, task: Task): number {
    if (!executor.expertise.length) return 0.5;
