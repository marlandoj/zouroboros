...
describe('MimirTransport', () => {
  ...
  test('forwards the configured gate token as a bearer credential', async () => {
    const originalFetch = globalThis.fetch;
    let authorization = '';
    ...
    try {
      const transport = new MimirTransport('http://localhost:7820', 'test-gate-token');
      const result = await transport.execute(makeTask(), defaultOptions);
      expect(result.success).toBe(true);
      expect(authorization).toBe('Bearer test-gate-token');
    } finally {
      globalThis.fetch = originalFetch;
    }
  });
