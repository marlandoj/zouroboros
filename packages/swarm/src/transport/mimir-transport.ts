...
function readGateToken(): string {
  if (process.env.ZO_GATE_TOKEN) return process.env.ZO_GATE_TOKEN;
  try {
    const secretsPath = process.env.ZO_SECRETS_PATH || '/root/.zo_secrets';
    const line = readFileSync(secretsPath, 'utf8')
      .split('\n')
      .find((candidate) => /^export\s+ZO_GATE_TOKEN=/.test(candidate));
    if (!line) return '';
    return line.replace(/^export\s+ZO_GATE_TOKEN=/, '').trim().replace(/^['\"]|['\"]$/g, '');
  } catch {
    return '';
  }
}
...
export class MimirTransport implements ExecutorTransport {
  ...
  constructor(gateUrl: string = 'http://localhost:7820', gateToken: string = readGateToken()) {
  ...
  async execute(task: Task, options: TransportOptions): Promise<TaskResult> {
    ...
    try {
      ...
      if (this.gateToken) headers.Authorization = `Bearer ${this.gateToken}`;
