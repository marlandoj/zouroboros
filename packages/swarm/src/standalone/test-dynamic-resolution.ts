...
let passed = 0;
let failed = 0;
let comboServiceAvailable = false;
let combos: any[] = [];

try {
  combos = await fetchCombos();
  comboServiceAvailable = combos.length > 0;
  console.log(`✅ Combo service reachable — ${combos.length} combos available`);
} catch (e: any) {
  console.log(`⚠️  Combo service unavailable — testing static fallback only (${e.message})`);
}
...
if (comboServiceAvailable) {
  console.log("Dynamic resolution was active for all non-override cases.");
} else {
  console.log("Static fallback was used (combo service not available).");
}
