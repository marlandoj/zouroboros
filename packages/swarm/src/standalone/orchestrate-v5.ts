...
// Learned credit assignment: capture the distant downstream outcome keyed by
// trace_id so reputation.ts can join it back to gate-vote participation.
import { captureOutcome } from "./trace-outcome";
...
Consider approaching this as a "${stagnation.suggestedPersona}" would.
  ...
  private createEpisode(successful: number, failed: number, durationMs: number): void {
    try {
      ...
      // Learned credit assignment: record this trace's outcome so reputation can
      // credit/blame the gate voters whose decisions correlated with it. First
      // write wins (idempotent on retry). Only when a trace_id is in scope.
      if (process.env.ZO_TRACE_ID) {
        captureOutcome(
          process.env.ZO_TRACE_ID,
          outcome as "success" | "failure",
          `swarm:${this.swarmId}`,
          { tasks: this.results.length, succeeded: successful, failed, elapsed_ms: durationMs }
        );
      }
      db.close();
    } catch {}
...
async function main(): Promise<void> {
  ...
  let tasks: Task[];
  try {
    const raw = JSON.parse(readFileSync(campaign, "utf-8"));
    if (!Array.isArray(raw)) {
      console.log("Campaign file must contain a JSON array of tasks");
      process.exit(1);
    }
    tasks = raw.map((t: unknown, i: number) => {
      try {
        return validateTask(t);
      } catch (err) {
        if (err instanceof SchemaValidationError) {
          console.log(`Task[${i}] validation failed: ${err.message}`);
        } else {
          console.log(`Task[${i}] validation failed: ${err}`);
        }
        process.exit(1);
      }
    });
