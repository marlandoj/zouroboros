...
import { resolveTraceId } from "./trace-bridge";
...
async function toolSwarmExecute(args: {
  ...
  const childEnv = traceId ? { ...process.env, ZO_TRACE_ID: traceId } : { ...process.env };
  ...
  if (args.waitForCompletion) {
    ...
    try {
      const result = await $`${cmd}`.env(childEnv).timeout(args.timeoutMinutes ? args.timeoutMinutes * 60 * 1000 : 600000);
