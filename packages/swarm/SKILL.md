...
description: Multi-agent orchestrator with token optimization, adaptive 6/8-signal composite routing, and persistent memory. Routes DAG tasks across Claude Code, Hermes, Gemini, Codex, OpenCode, Mimir, Kimi, and Pi through bridge, ACP, and native transports.
metadata:
  created: 2026-02-08
  updated: 2026-07-30
  version: 5.1.1
---
# Swarm Orchestrator Skill v5.1.1
...
**Current routing model:** Tasks execute through bridge, ACP, or native transports selected from the eight-executor registry. Six core signals cover capability matching, health, complexity fit, execution history, procedural learning, and episodic performance; budget and role signals are added when present. Auto-creates memory episodes after every swarm run.
...
```bash
# PRIMARY: Bun TS orchestrator (adaptive 6/8-signal routing, stagnation detection, OmniRoute, etc.)
...
|---------|--------|----------------|
| **v5 current** | ✅ **Current** | Eight executors, adaptive 6/8-signal routing, bridge/ACP/native transports, cascade mitigation, memory context injection, hierarchical telemetry |

---

## Architecture: Registry-Driven Execution

Tasks execute through the transport declared by each executor registry entry: bridge scripts, ACP adapters, or the native Mimir memory transport. Shared MCP configuration supplies authenticated Zo tools and memory services to ACP executors.
...
Query past runs:
```bash
bun ~/Skills/zo-memory-system/scripts/memory.ts episodes --entity "swarm.commerce-a" --since "7 days ago"
```
...
| File | Purpose |
|------|---------|
| `orchestrate-v5.ts` | Main orchestrator with adaptive 6/8-signal routing |
...
The orchestrator discovers eight executors through `executor-registry.json` and uses each entry's declared transport, capabilities, model router, MCP policy, and fallback chain.

| Executor | Transport | Primary use |
|----------|-----------|-------------|
| `claude-code` | ACP | Complex implementation, architecture, review |
| `hermes` | Native ACP | Research, external tools, autonomous investigation |
| `gemini` | Native ACP | Large-context, multimodal, UI analysis |
| `codex` | ACP | Fast repository implementation and refactoring |
| `opencode` | Native ACP | Vendor-neutral and provider-pinned coding |
| `kimi` | Native ACP bridge | Long-context coding and Moonshot-family review |
| `pi` | One-shot bridge with MCP adapter | Fast focused implementation and isolated comparisons |
| `mimir` | Native memory transport | Read-only historical and institutional context |

See [`docs/executors/capability-matrix.md`](docs/executors/capability-matrix.md) for tool access, hierarchical delegation boundaries, model routes, SDK corpus coverage, selection priority, and fallback behavior.
