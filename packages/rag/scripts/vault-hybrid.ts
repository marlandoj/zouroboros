...
import { MEMORY_DB, embed } from "./rag-core.ts";

const VAULT_ROOT = "/home/workspace";
const VAULT_DB = MEMORY_DB;
...
const RETIRED_DIRS = new Set([
  "CommerceA_Blueprint", // superseded by Notes/Commerce_Canon (cleaned 2026-05-17)
]);
...
function collectMarkdownFiles(dir: string, files: string[] = []): string[] {
  ...
  for (const entry of readdirSync(dir)) {
    ...
    if (RETIRED_DIRS.has(entry)) continue;
  ...
  const [semanticResults, graphResults] = await Promise.all([
    ...
    (async () => {
      try {
        ...
        const rows = vaultDb
          .query(
            `SELECT DISTINCT file_path, 0.5 AS score, 0 AS rank
             ...
             WHERE file_path IN (
               SELECT DISTINCT source_id FROM vault_links WHERE target_id LIKE ?
               UNION
               SELECT DISTINCT target_id FROM vault_links WHERE source_id LIKE ?
             )
             LIMIT ?`
          )
          .all(k, k, topK) as any[];
        ...
        // vault_embeddings may not exist yet; semantic-only fallback
        return [];
      }
    })(),
  ]);
