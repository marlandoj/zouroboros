...
const DIRS = {
  swarmResults: process.env.SWARM_RESULTS_DIR || `${process.env.HOME || "/root"}/.swarm/results`,
...
const DB_PATH = process.env.ZOUROBOROS_MEMORY_DB || process.env.ZO_MEMORY_DB || "/home/workspace/.zo/memory/shared-facts.db";
...
async function isSwarmInMemory(swarmId: string): Promise<boolean> {
  try {
    ...
    const row = db.query(`SELECT COUNT(*) as c FROM episodes WHERE id = ?`).get(`swarm.${swarmId}`) as { c: number } | null;
    db.close();
    return (row?.c ?? 0) > 0;
  } catch {
    return false;
...
async function captureSwarm(resultPath: string): Promise<"captured" | "skipped" | "error"> {
  return new Promise((resolve) => {
    const proc = spawn("bun", [SCRIPTS.swarm, "--post-swarm", resultPath], { stdio: "pipe" });
    let output = "";
    proc.stdout?.on("data", (data) => output += data.toString());
    proc.stderr?.on("data", (data) => output += data.toString());
    proc.on("close", (code) => {
      if (code !== 0) return resolve("error");
      if (output.includes("SWARM_CAPTURE_STATUS=captured")) return resolve("captured");
      if (output.includes("SWARM_CAPTURE_STATUS=skipped")) return resolve("skipped");
      resolve("error");
    });
  });
}

export async function maintainSwarmMemory(): Promise<{ captured: number; skipped: number; errors: number }> {
  ...
  for (const swarm of swarms) {
    ...
    const status = await captureSwarm(swarm.path);
    if (status === "captured") {
      captured++;
      await log(`   ✅ ${swarm.swarmId}`);
    } else if (status === "skipped") {
      skipped++;
      await log(`   ⏳ ${swarm.swarmId} (incomplete)`);
    } else {
      errors++;
      await log(`   ❌ ${swarm.swarmId}`);
    }
