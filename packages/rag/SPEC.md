...
• commerce-a-seo-audit (entity=swarm.commerce-a, 2026-03-25) — 4/6 tasks succeeded
```

### Implementation
- `scripts/rag-swarm-retrieval.ts` — runs post-swarm and query modes
...
bun scripts/rag-swarm-retrieval.ts --query "site review"

# Capture after swarm run. The canonical input is the base result path.
bun scripts/rag-swarm-retrieval.ts --post-swarm ~/.swarm/results/swarm_xxx.json
```

`--post-swarm` also accepts a bare swarm ID for compatibility. It resolves the base
result under `~/.swarm/results/` without prefixing absolute paths. A result is complete
when its JSON has a terminal `status`, or when a sibling/legacy `/dev/shm`
`<swarm-id>-complete.json` artifact exists. Non-terminal results are skipped cleanly.
...
[Semantic — 3 hits, score 0.82]
  → Notes/commerce-a/supplier-matrix.md  (0.91)
  → Projects/jhh-finance/cfee-tracker.ts  (0.74)
  → Skills/commerce-a-ops-sop/...        (0.71)

[Graph neighbors — 2 hits]
  ← Notes/commerce-a/vendor-contracts.md
  ← Notes/commerce-a/sku-canon.md
```
...
• commerce-a brand voice: educational, scientific, avoid hard-sell
...
### What It Does
1. **Swarm**: Scans base results in `~/.swarm/results/`, ignores completion artifacts, captures completed episodes once, and skips in-progress results without an error
...
- [x] `rag-swarm-retrieval.ts` — runs post-swarm and query modes
...
| Query swarm memory | `bun scripts/rag-swarm-retrieval.ts --query "task description"` |
