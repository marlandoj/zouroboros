...
export function loadConfig(configPath: string = DEFAULT_CONFIG_PATH): ZouroborosConfig {
  if (!existsSync(configPath)) {
    return applyEnvGates({ ...DEFAULT_CONFIG });
  }
 ...
 * endpoint).
 */
export function applyEnvGates(config: ZouroborosConfig): ZouroborosConfig {
  if (!config.memory.vectorEnabled) return config;
  if (process.env.ZOUROBOROS_VECTOR_FORCE === '1') return config;
