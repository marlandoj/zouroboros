...
describe('drift tolerance — pre-existing rich schemas (W3 follow-up)', () => {
  // Standalone init paths (fact-extractor.ts, conversation-capture.ts, etc.)
  // create `facts_fts` and `capture_log` with richer column lists than the
  // registry definitions. The migration chain must coexist with those —
  // 011 was previously failing on production DBs because its INSERT named
  // a column (fact_id) that doesn't exist in the rich variant; 012 was
  // failing because its index targeted a column (captured_at) that the
  // production schema doesn't have. Both patched 2026-05-05.
  test('migrate() succeeds when facts_fts already has rich schema', () => {
    db.exec(`
    ...
    `);
    db.run(
      `INSERT INTO facts (id, persona, entity, key, value, text)
       VALUES (?, ?, ?, ?, ?, ?)`,
      ['f1', 'shared', 'alice', 'role', 'engineer', 'alice is an engineer'],
    );

    const runner = createMigrationRunner(db);
    const result = runner.migrate();
    expect(result.errors).toEqual([]);
    expect(result.applied).toContain('011_add_facts_fts');

    // Schema preserved (still rich)
    const sql = (db
      .query("SELECT sql FROM sqlite_master WHERE name='facts_fts'")
      .all() as { sql: string }[])[0].sql;
    expect(sql).toContain('key');
    expect(sql).toContain('category');
  });

  test('migrate() succeeds when capture_log already has rich schema', () => {
    db.exec(`
    ...
    const result = runner.migrate();
    expect(result.errors).toEqual([]);
    expect(result.applied).toContain('012_add_capture_log');
  });
});
