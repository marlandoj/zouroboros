...
describe('applyEnvGates', () => {
  const savedKey = process.env.OPENAI_API_KEY;
  const savedZoKey = process.env.ZO_OPENAI_API_KEY;
  const savedForce = process.env.ZOUROBOROS_VECTOR_FORCE;

  beforeEach(() => {
    delete process.env.OPENAI_API_KEY;
    delete process.env.ZO_OPENAI_API_KEY;
    delete process.env.ZOUROBOROS_VECTOR_FORCE;
    _resetEnvGateWarning();
  });
  ...
  afterEach(() => {
    ...
    if (savedZoKey === undefined) delete process.env.ZO_OPENAI_API_KEY;
    else process.env.ZO_OPENAI_API_KEY = savedZoKey;
    if (savedForce === undefined) delete process.env.ZOUROBOROS_VECTOR_FORCE;
    else process.env.ZOUROBOROS_VECTOR_FORCE = savedForce;
  });

  test('disables vectorEnabled when no API key is set', () => {
    const config = structuredClone(DEFAULT_CONFIG);
    config.memory.vectorEnabled = true;
    const gated = applyEnvGates(config);
    expect(gated.memory.vectorEnabled).toBe(false);
  });

  test('keeps vectorEnabled when OPENAI_API_KEY is set', () => {
    process.env.OPENAI_API_KEY = 'sk-test-key';
    const config = structuredClone(DEFAULT_CONFIG);
    config.memory.vectorEnabled = true;
    const gated = applyEnvGates(config);
    expect(gated.memory.vectorEnabled).toBe(true);
  });

  test('keeps vectorEnabled when ZO_OPENAI_API_KEY is set', () => {
    process.env.ZO_OPENAI_API_KEY = 'sk-test-zo-key';
    const config = structuredClone(DEFAULT_CONFIG);
    config.memory.vectorEnabled = true;
    const gated = applyEnvGates(config);
    expect(gated.memory.vectorEnabled).toBe(true);
  ...
  test('respects ZOUROBOROS_VECTOR_FORCE override', () => {
    process.env.ZOUROBOROS_VECTOR_FORCE = '1';
    const config = structuredClone(DEFAULT_CONFIG);
    config.memory.vectorEnabled = true;
    const gated = applyEnvGates(config);
    expect(gated.memory.vectorEnabled).toBe(true);
  ...
  test('is a no-op when vectorEnabled is already false', () => {
    const config = structuredClone(DEFAULT_CONFIG);
  ...
  test('loadConfig applies gates on default path', () => {
    const config = loadConfig('/tmp/nonexistent-zouroboros/config.json');
    expect(config.memory.vectorEnabled).toBe(false);
  });

  test('mergeConfig applies gates', () => {
    const merged = mergeConfig({});
    expect(merged.memory.vectorEnabled).toBe(false);
  });
});
