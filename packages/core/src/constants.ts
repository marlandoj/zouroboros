...
// Portable default workspace root (host-independent). Runtime resolution
// honors ZOUROBOROS_WORKSPACE / ZO_WORKSPACE, then process.cwd() (see paths.ts);
// this constant only seeds config defaults and is never a host-specific path.
export const DEFAULT_WORKSPACE_ROOT = join(homedir(), '.zouroboros', 'workspace');
