...
import {
  ...
  measureRAGHealth,
  measureChunkHealth,
  measureEvalIntegrity,
  measureWiringHealth,
  measureEvalProductionParity,
  measureHoldoutFreshness,
} from './collector.js';
import { measureRAGHealthShadow } from './rag-health-shadow.js';

export async function buildScorecard(): Promise<Scorecard> {
  const timestamp = new Date().toISOString();

  const collectors: Array<Promise<MetricResult>> = [
    measureMemoryRecall(),
    measureGraphConnectivity(),
    measureRoutingAccuracy(),
    measureEvalCalibration(),
  ...
  if (process.env.RAG_HEALTH_SHADOW === '1') collectors.push(measureRAGHealthShadow());
  const metrics = await Promise.all(collectors);

  // Calculate composite score (weighted average). Weight-0 metrics (Eval-Integrity)
  // are display-only tripwires and contribute nothing to the composite by construction.
  const composite = metrics.reduce((sum, m) => sum + m.score * m.weight, 0) /
    metrics.reduce((sum, m) => sum + m.weight, 0);

  // Find weakest metric / opportunities among OPTIMIZED metrics only. Weight-0
  // tripwires must never hijack prescribe's weakest-metric selection — they inform
  // the human but are not optimization targets.
  const optimized = metrics.filter(m => m.weight > 0);

  const weakest = optimized.reduce((min, m) => m.score < min.score ? m : min, optimized[0]);

  // Build opportunities list (sorted by impact potential)
  const opportunities = optimized
...
export function formatScorecard(scorecard: Scorecard): string {
  ...
  for (const m of scorecard.metrics) {
    const statusIcon = m.status === 'HEALTHY' ? '✅'
      : m.status === 'WARNING' ? '⚠️'
