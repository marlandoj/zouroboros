...
import * as _realScorecard from '../introspect/scorecard.js';
import * as _realPlaybook from '../prescribe/playbook.js';
import * as _realGovernor from '../prescribe/governor.js';
import * as _realSeed from '../prescribe/seed.js';
import * as _realExecutor from '../evolve/executor.js';

// Snapshot the real modules BEFORE installing mocks. bun's mock.module is process-global
// and persists across files, so without restoring these in afterAll the stubs below leak
// into every sibling test that runs afterward (they were silently breaking trajectory /
// RAG-playbook / governor / curiosity / wiring suites). The spreads capture real exports
// while the registry is still real; afterAll re-installs those snapshots.
const REAL_MODULES: Array<[string, Record<string, unknown>]> = [
  ['../introspect/scorecard.js', { ..._realScorecard }],
  ['../prescribe/playbook.js', { ..._realPlaybook }],
  ['../prescribe/governor.js', { ..._realGovernor }],
  ['../prescribe/seed.js', { ..._realSeed }],
  ['../evolve/executor.js', { ..._realExecutor }],
];
...
class HookableSelfHeal extends SelfHeal {
  ...
  override async evolve(options: any = {}) {
    ...
    await this.hooks.emit('task.complete' as LifecycleEvent, {
      ...
      // The executor is mocked in this test to return an extra `iterations`
      // sentinel; the production EvolutionResult does not carry it.
...
// Restore the real modules so these mocks don't leak into sibling test files.
afterAll(() => {
