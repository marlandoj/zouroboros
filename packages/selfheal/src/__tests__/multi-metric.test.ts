...
describe('MultiMetricOptimizer', () => {
  ...
  describe('anti-Goodhart: default objective targets', () => {
    test('Wiring Health is an optimization target; Eval-Integrity is NOT', () => {
      const opt = new MultiMetricOptimizer(); // default objective

      // wouldImproveComposite reports a regression only for metrics in objective.targets.
      // Dropping Eval-Integrity must NOT register (it is not optimized).
      const evalDrop = opt.wouldImproveComposite(
        { 'Eval-Integrity': 1.0 },
        { 'Eval-Integrity': 0.0 },
        {}
      );
      expect(evalDrop.regressions.some(r => r.includes('Eval-Integrity'))).toBe(false);

      // Dropping Wiring Health MUST register as a regression (it is optimized).
      const wiringDrop = opt.wouldImproveComposite(
        { 'Wiring Health': 1.0 },
        { 'Wiring Health': 0.0 },
        {}
      );
      expect(wiringDrop.regressions.some(r => r.includes('Wiring Health'))).toBe(true);
    });

    test('default below-threshold constraints never name Eval-Integrity', () => {
      const opt = new MultiMetricOptimizer();
      // A scorecard where both signals sit at 0 — only Wiring Health (a target)
      // should surface as a below-threshold constraint, never Eval-Integrity.
      const scorecard = makeScorecard([
        makeMetric('Wiring Health', 0.0, 0.12),
        makeMetric('Eval-Integrity', 0.0, 0),
      ]);
      const plan = opt.buildPlan(scorecard, {});
      expect(plan.constraints.some(c => c.includes('Wiring Health'))).toBe(true);
      expect(plan.constraints.some(c => c.includes('Eval-Integrity'))).toBe(false);
    });
  });
