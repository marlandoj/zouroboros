...
import {
  ...
  type SelfHealPlanGateShadow,
} from "../prescribe/plan-gate.js";

const WORKSPACE = getWorkspaceRoot();
// Same INTROSPECT path reconciliation as standalone/evolve.ts (seed-antigoodhart-wiring,
// AC-S.2): the live-introspection fallback at main() must resolve to a real introspect that
// runs buildScorecard (incl. the new Eval-Integrity + Wiring Health collectors). The prior
// phantom Skills/zouroboros-introspect/ path did not exist and broke the --no-scorecard path.
...
const CONSENSUS_ENABLED = !!values.consensus;
...
interface Prescription {
  ...
  consensus?: { status: string; pass: boolean | null; detail: string };
    ...
    case "Eval Calibration":
      return isCritical
        ...
        : {
            ...
            targetFile: "packages/workflow/src/evaluate/semantic.ts",
            metricCommand: "echo 0",
            metricDirection: "lower_is_better",
            constraints: [
    ...
    case "Procedure Freshness":
      return isCritical
        ...
        : {
            ...
            description: "Trigger LLM-powered evolution on stale procedures",
            targetFile: null,
            metricCommand: `sqlite3 "${MEMORY_DB}" "SELECT CAST(SUM(CASE WHEN updated_at < datetime('now','-14 days') THEN 1 ELSE 0 END) AS FLOAT) / COUNT(*) * 100 FROM procedures;"`,
            metricDirection: "lower_is_better",
            constraints: [
...
async function main() {
  ...
  let program = generateProgram(targetMetric, playbook);

  // Step 5b: Run BGP consensus gate on the seed
  const gateLabel = `prescribe:${targetMetric.name}:${playbook.id}`;
  const consensus = runLegacyConsensusGate(seed, {
    criteria: "correctness,safety,design",
    label: gateLabel,
    workspaceRoot: WORKSPACE,
  });
  const planGateShadow = runSelfHealPlanGateShadow({
    seed,
    title: `Self-heal plan for ${targetMetric.name}`,
    taskTitle: playbook.name,
    legacy: consensus,
    workspaceRoot: WORKSPACE,
  });
  console.error(`Consensus gate: ${consensus.status}`);
  if (consensus.status === "escalate" || consensus.status === "rejected") {
    governor.flags.push(`BGP_CONSENSUS_${consensus.status.toUpperCase()}: voters disagreed on seed safety`);
    governor.approved = false;
    governor.requiresHuman = true;
    governor.riskLevel = "HIGH";
    governor.reason = `Consensus gate ${consensus.status} — voters flagged the seed; human review required`;
  } else if (consensus.status === "error" || consensus.status === "skipped") {
    governor.flags.push(`BGP_CONSENSUS_${consensus.status.toUpperCase()}: gate unavailable, proceeding without consensus signal`);
  }
  ...
  if (DRY_RUN) {
    ...
    // Consensus Gate: validate generated seed before writing
    if (CONSENSUS_ENABLED) {
      console.error("\n🔒 Running consensus gate on seed...");
      const seedGate = runLegacyConsensusGate(seed, {
        criteria: "correctness,security",
        label: prescription.id + "-seed",
        workspaceRoot: WORKSPACE,
      });
      if (seedGate.status === "rejected") {
        console.error(`❌ Consensus REJECTED seed for ${prescription.id}:\n${seedGate.detail.slice(0, 300)}`);
        console.error("   Aborting write — prescription discarded.");
        process.exit(2);
      } else if (seedGate.status === "escalate") {
        console.error(`⚠️  Consensus ESCALATE on seed for ${prescription.id}. Writing with caution flag.`);
      }
    }
    ...
    if (program) {
      ...
      if (CONSENSUS_ENABLED) {
        console.error("\n🔒 Running consensus gate on program...");
        const progGate = runLegacyConsensusGate(program, {
          criteria: "correctness,design",
          label: prescription.id + "-program",
          workspaceRoot: WORKSPACE,
        });
        if (progGate.status === "rejected") {
          console.error(`❌ Consensus REJECTED program for ${prescription.id}:\n${progGate.detail.slice(0, 300)}`);
          console.error("   Program discarded — seed preserved.");
          program = null;
        } else if (progGate.status === "escalate") {
          console.error(`⚠️  Consensus ESCALATE on program for ${prescription.id}. Writing with caution flag.`);
        }
      }
