...
async function measureGraphConnectivity(): Promise<MetricResult> {
  ...
  const linkedFacts = linkedMatch ? parseInt(linkedMatch[1]) : 0;
  ...
  // Blend linkedPct (does every fact have an edge?) with cohesion (are facts in one giant
  // component or scattered across many?). linkedPct alone saturates at 99%+ once auto-link
  // runs, masking heavy fragmentation. Cohesion penalizes high component-to-fact ratios.
  const fragRatio = linkedFacts > 0 ? components / linkedFacts : 1;
  const cohesion = Math.max(0, Math.min(1, 1 - fragRatio / 0.20));
  const blended = 0.6 * linkedPct + 0.4 * cohesion;

  const detail = `${(blended * 100).toFixed(1)}% (linked ${(linkedPct * 100).toFixed(1)}%, cohesion ${(cohesion * 100).toFixed(1)}%; ${total} facts, ${orphanCount} orphans, ${components} components)`;

  return buildMetric("Graph Connectivity", blended, 0.80, 0.55, 0.14,
    detail,
    blended < 0.80
      ? `Merge ${components} components: lower community-summarize --link-threshold (0.78→0.72) or backfill co-occurrence edges; ${orphanCount} orphans remain`
...
async function measureRoutingAccuracy(): Promise<MetricResult> {
  ...
  const query = `
    SELECT
      ...
      COUNT(*) as cnt
    FROM episodes e
    WHERE e.created_at > strftime('%s', 'now', '-14 days')
  ...
  if (!result.ok || !result.stdout.trim()) {
    ...
    const tableCheck = run(`sqlite3 "${MEMORY_DB}" "SELECT COUNT(*) FROM episodes WHERE created_at > strftime('%s', 'now', '-14 days');" 2>&1`);
    ...
    const generalQuery = `SELECT outcome, COUNT(*) FROM episodes WHERE created_at > strftime('%s', 'now', '-14 days') GROUP BY outcome;`;
...
async function measureEvalCalibration(): Promise<MetricResult> {
  ...
  // Look for eval report files in known locations + any Skills/*/evaluations/
  const evalDirs = [
    join(WORKSPACE, "evaluations"),
    join(WORKSPACE, "Zouroboros/evaluations"),
  ];
  // Scan Skills/*/evaluations/ directories
  const skillsDir = join(WORKSPACE, "Skills");
  if (existsSync(skillsDir)) {
    try {
      for (const skill of readdirSync(skillsDir)) {
        const evalDir = join(skillsDir, skill, "evaluations");
        if (existsSync(evalDir)) evalDirs.push(evalDir);
      }
    } catch { /* skip */ }
  }
  ...
  if (totalEvals === 0) {
    ...
    if (existsSync(MEMORY_DB)) {
      const evalEps = run(`sqlite3 "${MEMORY_DB}" "SELECT COUNT(*) FROM episodes WHERE summary LIKE '%eval%' AND created_at > strftime('%s', 'now', '-30 days');" 2>&1`);
...
async function measureEpisodeVelocity(): Promise<MetricResult> {
  ...
  const query = `
    SELECT
      SUM(CASE WHEN created_at > strftime('%s', 'now', '-7 days') AND outcome = 'success' THEN 1 ELSE 0 END) as recent_success,
      SUM(CASE WHEN created_at > strftime('%s', 'now', '-7 days') THEN 1 ELSE 0 END) as recent_total,
      SUM(CASE WHEN created_at <= strftime('%s', 'now', '-7 days') AND created_at > strftime('%s', 'now', '-14 days') AND outcome = 'success' THEN 1 ELSE 0 END) as prior_success,
      SUM(CASE WHEN created_at <= strftime('%s', 'now', '-7 days') AND created_at > strftime('%s', 'now', '-14 days') THEN 1 ELSE 0 END) as prior_total
    FROM episodes;
  `;
