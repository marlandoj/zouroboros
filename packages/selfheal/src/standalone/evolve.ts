...
} from "../introspect/holdout.js";
import { evaluateDrift } from "../prescribe/governor.js";
import {
  detectRegression,
  resolveDivergence,
  deriveVerifiedSuccess,
} from "../evolve/heldout-verification.js";
import { getScorecard as getScorecardShared } from "../evolve/scorecard.js";
import { recordIntervention } from "../evolve/intervention-ledger.js";

const WORKSPACE = getWorkspaceRoot();
// Anti-Goodhart wiring (seed-antigoodhart-wiring-2026-06-01, T7/AC-S.2): the loop's own
// scorecard fetch must resolve to an introspect that (a) exists on disk and (b) emits the
// full buildScorecard() metric set — including the new Eval-Integrity + Wiring Health
// collectors. The prior phantom path (Skills/zouroboros-introspect/scripts/introspect.ts)
// did not exist and silently broke getScorecard(); the legacy standalone/introspect.ts
// exists but predates buildScorecard and omits the new metrics. The library CLI below runs
// buildScorecard and resolves workspace deps when executed from WORKSPACE.
const INTROSPECT = join(WORKSPACE, "packages/selfheal/src/cli/introspect.ts");
const AUTOLOOP = join(WORKSPACE, "packages/workflow/src/cli/autoloop.ts");
...
function getScorecard(): ScorecardSnapshot | null {
  // Single-sourced (ZOU-882 B2): the introspect scorecard capture now lives in the shared
  // evolve/scorecard.ts so the library promotion path captures it the same way. Behavior
  // (command string, parse, null-handling) is identical to the prior inline version.
  return getScorecardShared(INTROSPECT, run);
}
...
async function main() {
  ...
  // Anti-Goodhart pre-flight (E1): snapshot the HIDDEN held-out bank the optimizer cannot
  // read. null = unmeasurable (runner/bun missing) → tripwire fail-safes off, never flags.
  const hiddenBaseline = scoreHiddenComposite();
  if (hiddenBaseline !== null) {
    console.error(`Hidden held-out baseline: ${(hiddenBaseline * 100).toFixed(1)}%`);
  }
  ...
  // Regression check (single-sourced — ZOU-882 B1)
  const regressionResult = detectRegression(baseline, postScorecard);
  const regression = regressionResult.regression;
  for (const drop of regressionResult.drops) {
    console.error(`⚠️  REGRESSION: ${drop.name} dropped ${(Math.abs(drop.delta) * 100).toFixed(1)}%`);
  }

  // Anti-Goodhart divergence (E1 / AC-E1.3): compare how the VISIBLE recall metric moved
  // against how the HIDDEN held-out bank (which the optimizer never reads) moved. A large
  // positive divergence — visible rose, hidden did not — is Goodhart drift. The held-out
  // bank mirrors the "Memory Recall" visible metric, so divergence is only meaningful when
  // that metric exists in both scorecards and the hidden bank is measurable.
  const divergenceReport: DivergenceReport | null = resolveDivergence({
    hiddenBaseline,
    baseline,
    post: postScorecard,
  ...
  if (divergenceReport) {
    console.error(
      `\n🔎 Held-out divergence: visible ${(divergenceReport.visibleDelta >= 0 ? "+" : "")}${(divergenceReport.visibleDelta * 100).toFixed(1)}% vs ` +
      `hidden ${(divergenceReport.hiddenDelta >= 0 ? "+" : "")}${(divergenceReport.hiddenDelta * 100).toFixed(1)}% ` +
      `(divergence ${(divergenceReport.divergence * 100).toFixed(1)}%)`
    );
    if (divergenceReport.goodhartFlag) {
      const drift = evaluateDrift(divergenceReport.divergence);
      console.error(`🚨 GOODHART DRIFT — ${drift.reason}`);
    }
  }
  ...
  const evolutionResult = {
    ...
    compositeAfter,
    regression,
    // AC-E1.3: Goodhart drift forces failure even when outcome metrics rose — the
    // visible gain is not trusted when the hidden held-out bank did not move with it.
    success: deriveVerifiedSuccess({ execSuccess: execResult.success, regression, goodhartFlag }),
    detail: execResult.detail,
    durationMs,
    visibleDelta: divergenceReport?.visibleDelta ?? null,
    hiddenDelta: divergenceReport?.hiddenDelta ?? null,
    divergence: divergenceReport?.divergence ?? null,
    goodhartFlag,
  ...
  // ZOU-280: persist the causal intervention → {state before, state after, delta} record
  // over the FULL scorecard, with the held-out drift flag joined onto the responsible
  // intervention (AC1 + AC3). Best-effort: a ledger write must never fail the evolve run.
  if (baseline) {
    try {
      recordIntervention({
        prescriptionId: rx.id,
        playbookId: rx.playbook.id,
        playbookName: rx.playbook.name,
        metricName: rx.metric.name,
        success: evolutionResult.success,
        reverted: regression,
        stateBefore: baseline,
        stateAfter: postScorecard,
        drift: divergenceReport
          ? {
              visibleDelta: divergenceReport.visibleDelta,
              hiddenDelta: divergenceReport.hiddenDelta,
              divergence: divergenceReport.divergence,
              goodhartFlag: divergenceReport.goodhartFlag,
            }
          : null,
      });
      console.error(`  [evolve] Intervention recorded to ledger${goodhartFlag ? " (drift flagged)" : ""}`);
    } catch (err: any) {
      console.error(`  [evolve] WARN: intervention ledger write failed: ${err?.message ?? err}`);
    }
  }
  ...
  if (existsSync(memoryTs)) {
    ...
    const outcome = goodhartFlag ? "failure" : evolutionResult.success ? "success" : regression ? "failure" : "ongoing";
    const driftEntity = goodhartFlag ? ",zouroboros.goodhart-drift" : "";
    run(`bun "${memoryTs}" episodes --create --summary "${summary.replace(/"/g, '\\"')}" --outcome ${outcome} --entities "zouroboros.evolution,zouroboros.${rx.metric.name.toLowerCase().replace(/\s+/g, "-")}${driftEntity}" --duration ${durationMs} 2>&1`);
  }

  // Final summary
  console.error("\n" + "═".repeat(56));
  if (goodhartFlag) {
    console.error(`🚨 GOODHART DRIFT — visible gain not mirrored by hidden held-out bank; escalated for human review`);
  } else if (evolutionResult.success) {
